// regra removida para documentação idela
...
export type HasItems<ItemType extends object> = {
  /** Lista de itens */
  items?: ItemType[];
  /** Chave dos itens, quando informado define o valor do item a ser exibido
   * @example "codigo", "descricao", ...
   */
  labelKey: keyof ItemType;
};

export type DefaultDropdownInputsProps<ItemType extends object> =
  HasItems<ItemType> &
    DefaultElementProps &
    HasButtons & {
      /**
       * Se informado, substitui a renderização padrão.
       */
      itemsRender?(item?: ItemType): ReactNode;
      /** Se habilitado exibe um filtro de pesquisa em ambas as listas. */
      filter?: boolean;
      /** Se informado, ele busca pelo tipo informado. */
      filterBy?: (keyof ItemType)[];
      /** Mensagem exibida quando nenhum item com as informações pesquisádas é encontrado */
